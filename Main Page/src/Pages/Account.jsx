...
const Account = () => {
    ...
    const updateShipping = async (data, cityName, districtName, wardName) => {
        ...
        await fetch(`${API_BASE_URL}/api/Users/updateShipping?userId=${decodedToken['Id']}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${jwtToken}`
            ...
            {user ? ( 
                <div style={{ width: '100%', display: 'flex', flexWrap: 'wrap', margin: '48px 0 48px 0' }}>
                    <div style={{ background: '#f5f5f7', width: '100%' }}>
                        <div style={{ width: viewportWidth > 1000 ? '66%' : '90%', margin: 'auto', display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e2e2e4' }}>
                        ...
                        <p style={{ width: viewportWidth > 1000 ? '66%' : '90%', margin: '25px auto', fontSize: '35px', color: 'black', fontFamily: 'SF-Pro-Display-Semibold' }}>Hi, {user.firstName}.</p>
                    </div>
                    <div style={{ width: '100%' }}>
                        <div style={{ width: viewportWidth > 1000 ? '66%' : '90%', margin: 'auto auto 35px auto', display: 'flex', flexWrap: 'wrap', overflow: 'overlay' }}>
                    ...
                    <div style={{ width: '100%', background: '#f5f5f7' }}>
                        <div style={{ width: '66%', margin: 'auto' }}>
                            <div style={{ width: viewportWidth > 1000 ? '40%' : '80%', height: '250px', padding: '40px 42px 40px 42px', background: 'white', margin: '50px 0 50px 0', borderRadius: '25px' }}>
